...
//           return (
//             <tr key={d._id} className="hover:bg-blue-50 transition-colors duration-200 cursor-pointer">
...
//               <td className="p-3 text-sm text-gray-900 whitespace-nowrap">{duree} jour(s)</td>
...
export const CongesManager: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 py-4 px-2 sm:py-6 sm:px-6 lg:py-8 lg:px-8 transition-all duration-300 animate-fadeIn">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 p-4 sm:p-6 bg-white rounded-xl shadow-md hover:shadow-xl transition-shadow duration-300">
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-blue-700 bg-gradient-to-r from-blue-400 to-blue-800 bg-clip-text text-transparent text-center md:text-left">
            Gestion des Congés
          </h1>
          <p className="text-gray-600 mt-2 md:mt-0 text-sm sm:text-base text-center md:text-right">Gérez les demandes de congé de vos employés</p>
        </div>

        {/* Notifications flottantes */}
        {notification && (
          <div
            className={`fixed top-4 right-2 left-2 sm:left-auto sm:right-4 z-50 max-w-sm mx-auto sm:mx-0 w-full sm:w-96 shadow-lg rounded-lg p-4 sm:p-5 flex items-start space-x-3 animate-slideInRight ${
              notification.type === "success" ? "bg-green-500 text-white" : "bg-red-300 text-red-900"
            }`}
          >
            ...
            <button onClick={() => setNotification(null)} className="text-current hover:opacity-70 focus:outline-none text-lg">
              &times;
            </button>
          </div>
        )}
        ...
        {/* Notifications employés */}
        <div className="fixed bottom-4 right-2 left-2 sm:left-auto sm:right-4 z-40 space-y-3">
          {employeeNotifications.map((notif, index) => (
            <div
              key={index}
              className={`p-3 sm:p-4 rounded-lg shadow-lg flex items-start space-x-3 transform transition-all duration-500 ${
                notif.type === "success" ? "bg-green-600 border-l-4 border-green-800 text-white" : "bg-red-300 border-l-4 border-red-700 text-red-900"
              }`}
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              ...
              <div className="flex-1">
                <p className="text-xs sm:text-sm font-medium">Notification envoyée</p>
                <p className="mt-1 text-xs opacity-90">{notif.message}</p>
              </div>
            </div>
          ))}
        ...
        <div className="mb-4 flex justify-center sm:hidden">
          <button
            onClick={() => setShowConges(!showConges)}
            className="bg-blue-500 hover:bg-blue-600 text-white font-medium py-2 px-4 rounded-lg flex items-center transition-colors duration-300 text-sm"
          >
            {showConges ? "Masquer les congés" : "Voir les congés"}
            <svg className="ml-2 w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round">
              <path d={showConges ? "M5 15l7-7 7 7" : "M19 9l-7 7-7-7"} />
        ...
        <div className={`${showConges ? "block" : "hidden"} sm:block`}>
          ...
          {/* Pie Chart */}
          <div className="bg-white p-4 sm:p-6 rounded-xl shadow-md border border-blue-200 mb-6 h-72">
            <h3 className="text-lg sm:text-xl font-semibold text-blue-700 mb-4 text-center">Répartition des demandes de congé</h3>
            <Pie data={pieData} options={pieOptions} />
          </div>
...
const StatCard: React.FC<StatCardProps> = ({ title, count, status }) => {
  ...
  return (
    <div className={`bg-white rounded-xl shadow-md p-4 border border-blue-200 hover:shadow-lg hover:scale-105 transition-transform duration-300`}>
      <div className="flex items-center space-x-3">
        <div className={`rounded-full ${config.iconBg} p-2 sm:p-3`}>
          {config.iconSvg}
        </div>
        <div>
...
const DemandesTable: React.FC<any> = ({ demandes, getEmploye, getStatut, handleApprove, handleReject, showDetails, getButtonClasses }) => {
  return (
    <div className="overflow-x-auto bg-white rounded-xl shadow-md p-4 sm:p-5 border border-blue-200">
      <h3 className="text-base sm:text-lg font-semibold text-blue-700 mb-3">Liste des demandes de congé</h3>
      <div className="overflow-x-auto">
        <table className="min-w-full border-collapse">
          <thead>
            <tr className="bg-blue-100 rounded-lg">
              <th className="p-2 sm:p-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wide rounded-tl-lg">Employé</th>
              <th className="p-2 sm:p-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wide">Période</th>
              <th className="p-2 sm:p-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wide hidden sm:table-cell">Type</th>
              <th className="p-2 sm:p-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wide">Durée</th>
              <th className="p-2 sm:p-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wide">Statut</th>
              <th className="p-2 sm:p-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wide rounded-tr-lg">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-blue-200">
            {demandes.length > 0 ? (
              demandes.map((d: DemandeConge) => {
                ...
                return (
                  <tr key={d._id} className="hover:bg-blue-50 transition-colors duration-200 cursor-pointer">
                    ...
                    <td className="p-2 sm:p-3 text-xs sm:text-sm text-gray-900">
                      {new Date(d.dateDebut).toLocaleDateString("fr-FR")} - {new Date(d.dateFin).toLocaleDateString("fr-FR")}
                    </td>
                    <td className="p-2 sm:p-3 text-xs sm:text-sm text-gray-900 hidden sm:table-cell">{d.typeConge}</td>
                    <td className="p-2 sm:p-3 text-xs sm:text-sm text-gray-900">{duree} jour(s)</td>
              ...
              <tr>
                <td colSpan={6} className="p-4 sm:p-6 text-center text-gray-500 text-xs sm:text-sm">
                  Aucune demande de congé
                </td>
              </tr>
            )}
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4">
      <div className="bg-white rounded-xl shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-4 py-3 sm:px-6 sm:py-4 space-y-3 text-xs sm:text-sm">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            {demande.commentaireResponsable && (
              <>
                <div><strong className="text-gray-700">Commentaire :</strong></div>
                <div className="text-gray-900 col-span-1 sm:col-span-2">{demande.commentaireResponsable}</div>
              </>
            )}
          </div>
        </div>
        {statut === "En attente" && (
          <div className="px-4 py-3 sm:px-6 sm:py-4 border-t border-gray-200 flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-3">
            <button
              onClick={() => { handleApprove(demande); onClose(); }}
              className="px-3 py-2 sm:px-4 sm:py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors duration-200 text-xs sm:text-sm"
            >
              Valider
            </button>
            <button
              onClick={() => { handleReject(demande); onClose(); }}
              className="px-3 py-2 sm:px-4 sm:py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors duration-200 text-xs sm:text-sm"
            >
              Refuser
            </button>
          </div>
