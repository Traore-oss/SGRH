...
const API_BASE = "http://localhost:8000/api/recrutements";
...
const Recrutement = () => {
  ...
  const [showCandidatureForm, setShowCandidatureForm] = useState<string | null>(null);
  ...
  const handleCreateOffre = async () => {
    try {
      const res = await axios.post(API_BASE, newOffre);
  ...
  const handleUpdateOffre = async () => {
    ...
    try {
      const res = await axios.put(`${API_BASE}/${editingOffre._id}`, editingOffre);
      setOffres(offres.map(offre => offre._id === editingOffre._id ? res.data.updated : offre));
  ...
  // === Ajouter un candidat ===
  const handleAddCandidat = async (offreId: string) => {
    try {
      const res = await axios.post(`${API_BASE}/${offreId}/candidats`, newCandidat);
      setOffres(offres.map(offre => offre._id === offreId ? res.data.offre : offre));
  ...
  const toggleCandidatureForm = (offreId: string) => {
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="bg-white shadow-md rounded-lg p-6">
        ...
        <textarea
          ...
          className="border border-gray-300 p-3 rounded w-full h-32 mt-4 focus:ring-2 focus:ring-blue-500"
        />
        <div className="mt-4">
          <button
              ...
              {editingOffre && editingOffre._id === offre._id ? (
                <div className="mb-4">
                  ...
                  <div className="flex space-x-2">
                    <button onClick={handleUpdateOffre} className="bg-green-600 text-white px-4 py-2 rounded">Enregistrer</button>
                    <button onClick={cancelEditing} className="bg-gray-600 text-white px-4 py-2 rounded">Annuler</button>
                  </div>
                </div>
              ) : (
                <>
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      {offre.departement && <p className="text-gray-600">Département: {offre.departement}</p>}
                    ...
                    <div className="flex space-x-2">
                      <button onClick={() => startEditing(offre)} className="bg-yellow-500 text-white px-3 py-1 rounded text-sm">Modifier</button>
                      <button onClick={() => handleDeleteOffre(offre._id)} className="bg-red-600 text-white px-3 py-1 rounded text-sm">Supprimer</button>
                      <button onClick={() => toggleCandidatureForm(offre._id)} className="bg-blue-100 text-blue-700 px-4 py-2 rounded-lg font-medium hover:bg-blue-200">Postuler</button>
                    </div>
                  </div>
                  ...
                  {showCandidatureForm === offre._id && (
                    <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <input type="text" placeholder="Prénom" value={newCandidat.prenom} onChange={(e) => setNewCandidat({ ...newCandidat, prenom: e.target.value })} className="border p-2 rounded" />
                        <input type="text" placeholder="Nom" value={newCandidat.nom} onChange={(e) => setNewCandidat({ ...newCandidat, nom: e.target.value })} className="border p-2 rounded" />
                        <input type="email" placeholder="Email" value={newCandidat.email} onChange={(e) => setNewCandidat({ ...newCandidat, email: e.target.value })} className="border p-2 rounded" />
                        <input type="text" placeholder="URL du CV" value={newCandidat.cvUrl} onChange={(e) => setNewCandidat({ ...newCandidat, cvUrl: e.target.value })} className="border p-2 rounded" />
                      </div>
                      <div className="flex justify-end space-x-2 mt-4">
                        <button onClick={() => setShowCandidatureForm(null)} className="px-4 py-2 border border-gray-300 rounded">Annuler</button>
                        <button onClick={() => handleAddCandidat(offre._id)} className="bg-blue-600 text-white px-4 py-2 rounded">Envoyer</button>
                      </div>
                    </div>
                  )}
                  ...
                  {offre.candidats && offre.candidats.length > 0 && (
                    <div className="mt-4">
                      <h4 className="font-semibold text-gray-700 mb-2">Candidats ({offre.candidats.length})</h4>
                            ...
                            {offre.candidats.map((candidat, i) => (
                              <tr key={i} className="border-b hover:bg-gray-50">
                                ...
                                <td className="py-3 px-6">
                                  {candidat.cvUrl ? (
                                    <a href={candidat.cvUrl} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">
                                      Voir le CV
                                    </a>
                                ...
                                <td className="py-3 px-6">
                                  <span className={`px-2 py-1 rounded-full text-xs ${
                                    candidat.statutCandidature === 'Retenu'
                                      ? 'bg-green-100 text-green-800'
                                      : candidat.statutCandidature === 'Rejeté'
                                      ? 'bg-red-100 text-red-800'
